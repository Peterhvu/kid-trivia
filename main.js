...
function loadCards() {
    ...
    for (let index = 0; index < getCount(); index++) {
        ...
        swiper.appendSlide([
            `<div class="swiper-slide">Question: ${card.q}</div>`,
            `<div class="swiper-slide">Answer: ${card.a}</div>`,
            `<div class="swiper-slide">Fact: ${card.f}</div>`
        ]);
    }
    addLastSlide();
}
...
function addLastSlide() {
    let msg = (getCount() < 1) ? "This is the end." : "Tap for a new round.";
